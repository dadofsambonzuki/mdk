...
impl SqliteAccountStorage {
    ...
    pub fn replace_subscription_replay_obligations(
        ...
        self.connection.with_transaction(|| {
            ...
            for ((_, route), floor) in validated.iter().zip(inherited_floors) {
                prepared.push(self.prepare_validated_subscription_replay(route, floor)?);
            }
    ...
    fn prepare_validated_subscription_replay(
        ...
        conn.execute_cached(
            "INSERT INTO subscription_replay_obligations (
                ...
                endpoint_scope_digest, replay_floor
             ) VALUES (?1, ?2, ?3, ?4, ?5, ?6)
             ON CONFLICT(
